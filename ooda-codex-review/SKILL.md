...
args:
  ...
  - name: --restart-from-floor
    description: "Side-effect (low-level primitive). Records a `RestartFromFloor` event. The resolution line is always emitted; per the one-invocation-one-run model the floor IS the level passed via `--level`, so this renders e.g. as `restarted to floor: low`. Outcome `Idle` (exit 1). Prefer `--mark-retro-changes` for the canonical retrospective-changes flow (which also records the restart and additionally captures a REASON string); use `--restart-from-floor` only when no retrospective context applies (tests, recovery)."
  - name: -h, --help
    description: >
...
| 7    | `StuckCapReached`   | Loop mode: `--max-iter` iterations elapsed without a halt. The stderr header carries the action kind and blocker key of the **last attempted action**. When `last_attempted` is `AwaitReviews` (a polling Wait), the runner first attempts a cap-trip force-projection: one final observe → project pending slots as `Abandoned` → re-run decide. If decide produces a Halt (typically `HandoffAgent: AddressBatch` for any completed reviews + the abandoned slot, or `DoneFixedPoint` at ceiling) that Halt is returned instead, so the completed verdicts are surfaced rather than discarded. `StuckCapReached` only fires when the cap trips on a non-Wait action (where projection does not apply) or when the cap-trip observe / projection itself fails. Pair this with the mid-loop alive/idle discriminator: pending slots quiet for >90s are projected mid-loop (so a truly hung slot halts quickly), while continuously alive slots that just outrun the iter cap project at the trip. |
...
| 130  | `SignalInterrupted` | `SIGINT` (`128 + 2`). The loop polls `SHUTDOWN_SIGNAL` at each iteration boundary; on a trapped signal it appends a terminal `run_halted` event, releases the live marker, prints `Interrupted: exit code 130` on stderr, and exits 130 itself. Treat as clean shutdown, not a crash. The shell synthesizes the same `128 + 2` for an uncaught signal, so callers cannot distinguish trapped from kernel paths on `$?` alone.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                     |
| 143  | `SignalInterrupted` | `SIGTERM` (`128 + 15`). Same handling as `SIGINT`; stderr header is `Interrupted: exit code 143` and the binary exits 143.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                        |
...
| `--restart-from-floor`  | `restarted to floor: <FLOOR>` _(no separate at-floor row — the line is always emitted, e.g. `restarted to floor: low`; the at-floor no-op is not silent. The pre-restart level is orchestrator state, not this binary's, so it does not appear.)_                                                                                                                                                         |
...
| `--mark-retro-changes`  | `retrospective surfaced changes ("<REASON>"); restarted to floor: <FLOOR>` _(no separate at-floor row — the line is always emitted, e.g. `retrospective surfaced changes ("..."); restarted to floor: low`; the at-floor no-op is not silent. The trailing `restarted to floor: <FLOOR>` segment matches the `--restart-from-floor` template above so a single parser handles both flags' restart-tail.)_ |
