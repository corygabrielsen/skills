...
args:
  ...
  - name: --criteria STRING
    description: "Free-form review prompt forwarded to `codex review` as a positional argument inserted after codex's own target flag (which this binary derives from its own mode flag — `--uncommitted` becomes `codex review --uncommitted`, `--base BRANCH` becomes `codex review --base BRANCH`, `--commit SHA` becomes `codex review --commit SHA`, `--pr NUM` becomes `codex review --pr NUM`). Default omitted; codex uses its built-in criteria. Effective in loop mode only. Side-effect invocations skip the OODA loop and never spawn codex, so a `--criteria` value passed alongside a side-effect flag is parsed and ignored (NOT a UsageError — `--criteria` differs from `--fresh` here: `--fresh` is rejected because it would mutate a brand-new manifest with no history, while `--criteria` has no recorder side effect and is purely a subprocess prompt). `--criteria` is not part of the resume key, so passing different values across invocations affects only the next loop's spawned reviewers, not whether the recorder resumes."
  - name: --fresh
  ...
  - name: -h, --help
    description: Print usage to stdout and exit 0. Standalone short-circuit; not an invocation mode and does not require a mode flag. To distinguish from `DoneFixedPoint` (also exit 0) the canonical method is to check for `--help` / `-h` in the argv the orchestrator itself issued — see "Two invocation modes" below. As a defensive fallback (e.g. for orchestrators that don't track their own argv), checking whether stderr is empty also works — every non-`--help` Outcome (including `DoneFixedPoint` from either loop or side-effect mode) writes a header to stderr, so empty stderr uniquely identifies `--help`. Prefer the argv check when feasible: it is local to the orchestrator and does not depend on capturing stderr.
---
...
- **Side-effect mode** — triggered by `--mark-*` or the low-level
  ...
  alongside a side-effect flag (NOT a UsageError): `--criteria`
  (purely a subprocess prompt) and `--max-iter` (purely the loop
  iteration cap). Both differ from `--fresh` in this respect —
  `--fresh` is rejected because it would mutate a brand-new
  manifest with no history. See the `--criteria` arg description
  for the rationale. All non-help invocations require a mode flag
  for context (the recorder resume key is `(target, start_level)`;
  see Resume semantics).
...
| `<message>`     | Caught-error message embedded in `BinaryError: <message>`. Covers every `BinaryError` trigger source from the Outcomes table: codex spawn failure (loop mode only, since side-effect mode never spawns codex), IO error, recorder open failure, recorder record failure (loop mode and side-effect mode).                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                               |
...
| `--mark-address-passed` | `address passed at floor (<LEVEL>) (<COUNT> review(s) with issues); no drop` _(at configured floor; matches the `at floor (<LEVEL>)` shape used by `--drop-level`'s no-op row above)_                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                       |
