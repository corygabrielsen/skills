...
pub enum DecisionHalt {
    /// No actions to dispatch, no blockers — review has reached
    /// fixed point at the current ceiling.
    Success,
    /// Review is in a terminal lifecycle state (fixed point reached
    /// or aborted by user).
    Terminal(Terminal),
...
impl DecisionHalt {
    ...
    pub fn name(&self) -> &'static str {
        match self {
            ...
            Self::Terminal(Terminal::FixedPoint) => "Terminal(FixedPoint)",
...
pub enum Terminal {
    /// All n reviews clean at the configured ceiling reasoning
    /// level, and retrospective produced no architectural changes.
    FixedPoint,
    /// User aborted the review loop (e.g., SIGINT, --abort flag).
    Aborted,
}
