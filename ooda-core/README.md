...
| `atomic_io`, `file_lock`, `spawn`                                                        | `tmp+rename` writes with `0o700` directories; advisory `FileLock`; `run_with_limits` (deadline + per-stream byte caps, child spawned in its own process group and the whole group killed on breach) for subprocesses.                                                           |
