...
args:
  ...
  - name: --repo-root PATH
    description: Target working tree for every `gt`/`git`/codex subprocess. Default derived from CWD via `git rev-parse --show-toplevel`; invocations from outside any git tree are rejected as UsageError unless this flag is supplied.
