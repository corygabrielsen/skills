...
|  3   | `HandoffHuman(action)`    | `Hand off to human: <ActionKind>` (followed by `  see: <path>` pointer) | Read the prompt body from the pointed-to blob (`runs/<run-id>/blobs/<sha>.md`). Surface the handoff to the user (see "Surface to the user" below). Re-invoke `/ooda-pr` after they resolve it.                                                                                                                                                                   |
|  4   | `HandoffAgent(action)`    | `Hand off to agent: <ActionKind>` (followed by `  see: <path>` pointer) | Read the prompt body from the pointed-to blob (`runs/<run-id>/blobs/<sha>.md`). Surface the handoff to the user (see "Surface to the user" below), then dispatch an agent with the prompt body as input. Re-invoke `/ooda-pr` after the agent finishes.                                                                                                          |
...
`<state-root>/runs/<run-id>/blobs/<sha>.md` (content-addressed),
...
enforce non-emptiness; the blob may be zero-length. Embedded
newlines in the description appear verbatim in the blob.

**Surface to the user.** After reading the handoff blob, the calling
...
(3) the handoff blob body in full. The body already carries the
...
handoff blob body in item (3); items (1)-(2) are unchanged.
...
```
HandoffAgent: Rebase
  see: /home/user/.local/state/ooda/runs/20260516T120000Z-000000000-p1234/blobs/<sha>.md
```
...
Multi-line prompt example. The handoff blob carries the dashboard
