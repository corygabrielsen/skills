...
//! `LoopError → Outcome` conversion — the merged binary's
//! `LoopError` carries CodexObserve in addition to the PR-side
//! variants, so it can't share a blanket impl.

use crate::decide::action::ActionKind;
use crate::runner::LoopError;

/// PR-plus-codex-review-domain `Outcome`. 1:1 variant → exit-code
/// via [`ooda_core::Outcome::exit_code`].
