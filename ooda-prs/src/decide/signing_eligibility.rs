...
///
/// Returns at most one candidate. Empty result attests "either the
/// branch does not require signed commits, or every commit on HEAD
/// is verified — no signing pathology."
...
mod tests {
    ...
    fn silent_when_signing_required_but_all_verified() {
        let s = projection_with(true, vec![]);
        assert!(signing_eligibility_candidates(&s).is_empty());
    }

    #[test]
    fn fires_pathology_when_signing_required_and_unsigned() {
        let s = projection_with(true, vec![sha('a')]);
    ...
    fn payload_carries_every_unsigned_sha() {
        let s = projection_with(true, vec![sha('a'), sha('b'), sha('c')]);
        let cs = signing_eligibility_candidates(&s);
    ...
    fn blocker_key_is_stable_across_calls() {
        let s1 = projection_with(true, vec![sha('a')]);
        let s2 = projection_with(true, vec![sha('a'), sha('b'), sha('c')]);
        assert_eq!(
    ...
    #[test]
    fn fires_even_when_other_merge_state_signals_look_healthy() {
        // signatures_required + unsigned must fire regardless of
        // mergeStateStatus — the absence is broken whether or not
        // GitHub's host signal currently agrees.
        let mut s = projection_with(true, vec![sha('a')]);
        s.merge_state_status = MergeStateStatus::Clean;
        assert_eq!(signing_eligibility_candidates(&s).len(), 1);
