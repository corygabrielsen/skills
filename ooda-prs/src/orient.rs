...
/// its own variant. An `Option` axis (`copilot`, `cursor`) reserves
/// `None` to encode *no axis applicable here* — distinct from
/// *axis applies but is currently quiet*. Collapsing those two
/// onto a single quiet-state would readmit the false-halt bug the
/// optionality is here to forbid.
