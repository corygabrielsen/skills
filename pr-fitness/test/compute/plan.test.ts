...
describe("plan", () => {
  ...
  it("prescribes wait for pending CI", () => {
    ...
    const actions = plan(ci, APPROVED_REVIEWS, CLEAN_STATE, "pass");
  ...
  it("prescribes address_threads", () => {
    ...
    const actions = plan(CLEAN_CI, reviews, CLEAN_STATE, "pass");
  ...
  it("omits request_approval when other blockers exist", () => {
    ...
    const actions = plan(ci, reviews, CLEAN_STATE, "pass");
    assert.ok(!actions.some((a) => a.type.kind === "request_approval"));
  });
  ...
  it("orders CI before reviews before metadata", () => {
    ...
    const state: PrState = { ...CLEAN_STATE, content_label: false };
    const actions = plan(ci, reviews, state, "pass");
